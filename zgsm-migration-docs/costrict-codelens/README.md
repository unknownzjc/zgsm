...
主要的代码透镜提供者，实现了 VS Code 的 [`CodeLensProvider`](src/core/costrict/codelens/CodeLensProvider.ts:18) 接口。
...
import { MyCodeLensProvider } from "./src/core/costrict/codelens"
...
```typescript
import { CODELENS_FUNC } from "../costrict/base/common/constant"
...
通过 [`LangSetting`](src/core/costrict/codelens/CodeLensProvider.ts:24) 控制各语言的代码透镜行为：
...
```typescript
import { Logger } from "../costrict/base/common/log-util"
...
1. 在 [`CODELENS_FUNC`](src/core/costrict/codelens/CodeLensCallbacks.ts:11) 中添加配置
2. 实现对应的命令处理函数
3. 在语言类中添加显示条件检查
4. 更新配置选项
